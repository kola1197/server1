...
class Server:
    ...
    def connecting(self):  # подключение новых пользователей
        ...
        while self.is_connected:
            print("waiting new clint")
            conn, addr = self.serv.accept()
            print(conn)
            print(addr)
            if addr not in self.clients:
                self.createUser(addr, conn)
    ...
    def receiving(self, user):
        while self.is_connected:
            ...
            self.print_on_server(('from ' + user.nick + ' : ' + msg).encode('utf-8'))
            for u in self.users:
                if not u.nick == user.nick:
                    self.send_to_client(u.sock, user.nick + ": "+self.last_msg.decode('utf-8'))

    def createUser(self, addr, conn):
        user = User()
